...
public class NativeHeroResourceIT {
    ...
    void shouldNotAddInvalidItem() {
        ...
        given().body(hero.encode()).header(CONTENT_TYPE, APPLICATION_JSON).header(ACCEPT, APPLICATION_JSON).when()
    ...
    void shouldAddAnItem() {
        ...
        String location = given().body(hero.encode()).header(CONTENT_TYPE, APPLICATION_JSON)
                .header(ACCEPT, APPLICATION_JSON).when().post("/api/heroes").then().statusCode(CREATED.getStatusCode())
        ...
        given().pathParam("id", heroId).when().get("/api/heroes/{id}").then().statusCode(OK.getStatusCode())
                ...
                .body("otherName", Is.is(DEFAULT_OTHER_NAME)).body("level", Is.is(60))
    ...
    void shouldUpdateAnItem() {
        ...
        given().body(hero.encode()).header(CONTENT_TYPE, APPLICATION_JSON).header(ACCEPT, APPLICATION_JSON).when()
                ...
                .body("name", Is.is(UPDATED_NAME)).body("otherName", Is.is(UPDATED_OTHER_NAME)).body("level", Is.is(21))
                .body("picture", Is.is(UPDATED_PICTURE)).body("powers", Is.is(UPDATED_POWERS));
